...
const Card: React.FC<CardProps> = ({ card, handleCardClick }) => {
  ...
  return (
    <div
      key={card.id}
      className={twMerge(
        clsx("w-60 h-60 flex items-center justify-center border rounded bg-cover bg-center", {
          "bg-green-500 text-white": card.matched,
          "bg-red-500 text-white": card.error,
          "bg-blue-500 text-white": card.flipped && !card.matched && !card.error,
          "bg-gray-200": !card.flipped && !card.matched && !card.error && !backImage,
        })
      )}
      style={{
        backgroundImage: !card.flipped && !card.matched && !card.error && backImage
          ? `url(${backImage})`
